...
func _process(delta):
	...
	$PlayerMoveTimerVisual.material.set_shader_parameter("fill_ratio", $PlayerMoveTimer.time_left)
	...
	if started == true and player_alive == false:
		...
		if $Camera2D.position.y > -900:
			...
			if $Camera2D/FadeColor.visible == false:
				...
				await $Camera2D/FadeAnimation.animation_finished
...
func show_moves():
	...
	if player_choice == "SHOOT" and random_number == 1:
		...
		$Music.stop()
		$WinSound.play()
		await $Background/Foreground/Enemy.animation_finished
		$FallSoundFar.play()
		$Camera2D/FadeColor.visible = true
...
func _on_Fire_released():
	...
	match player_bullets:
		1:
			$Cylinder.play_backwards("Empty-One")
		2:
			$Cylinder.play_backwards("One-Two")
		3:
			$Cylinder.play_backwards("Two-Three")
		4:
			$Cylinder.play_backwards("Three-Four")
		5:
			$Cylinder.play_backwards("Four-Five")
		6:
			$Cylinder.play_backwards("Five-Six")
...
func _on_Restart_released():
	...
	$Camera2D/FadeColor.color = "ffffff00"
